...
// Add { sessions = [] } inside the parentheses
const Sidebar = ({ sessions = [],onNewChat } ) => {
    return (
        <div className="flex flex-col h-screen w-64 bg-[#0a192f] text-white overflow-hidden">
            ...
            <ul className='w-full flex flex-col'>
                <li 
                    onClick={onNewChat}
                    className='flex mr-3 bg-[#11234a] mt-3 p-2.5 rounded-br-[5px] rounded-tr-[5px] hover:cursor-pointer hover:bg-[#132d68] transition-all group'
                >
            ...
            <div className='flex-1 overflow-y-auto mt-4 
                ...
                <ul className='flex flex-col gap-1 px-2'>
                    {/* Now sessions is defined! */}
                    {sessions.map((chat, i) => (
                        <li key={chat.id || i} className=' mr-1 p-2.5 text-sm text-gray-300 hover:bg-[#11234a] hover:text-white rounded-md cursor-pointer transition-colors'>
                            {chat.title || `Chat History ${i + 1}`}
                        </li>
                    ))}
                </ul>
            </div>
