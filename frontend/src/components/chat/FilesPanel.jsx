    ...
    return (
        <div className='flex flex-col h-screen w-64 bg-[#0a192f] text-white border-l border-gray-800 overflow-hidden'>
            <div className='p-4 border-b border-gray-800'>
                <h2 className='font-bold text-sm uppercase tracking-widest text-blue-400'>Uploaded Files</h2>
            </div>

            <div className='flex-1 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-[#132d68] scrollbar-track-transparent'>
                <ul className='flex flex-col gap-2'>
                    {files.length === 0 ? (
                        <p className="text-gray-500 text-xs text-center mt-10">No files uploaded yet.</p>
                    ) : (
                        files.map((file) => (
                            <li key={file.id} className='p-2 bg-[#11234a]/50 rounded text-xs truncate border border-white/5 hover:border-blue-500/50 transition-all group'>
